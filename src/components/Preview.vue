<template>
    <div class="preview">
        <div class="row">
            ...
            <div class="col-sm-6">
                <!-- Here lies the images -->
                <div id="demo" class="carousel slide" data-ride="carousel">
                    <!-- Indicators -->
                    <ul class="carousel-indicators">
                        <template v-for="single_media in media">
                            <template v-if="single_media.id == 0">
                                <li data-target="#demo" :data-slide-to="single_media.id" class="active"></li>
                            </template>
                            <template v-else>
                                <li data-target="#demo" :data-slide-to="single_media.id"></li>
                            </template>
                        </template>
                            ...
                            <template v-if="single_media.id == 0">
                                <div class="carousel-item active">
                                    <template v-if="single_media.type == 'image'">
                                        <img class="img-responsive single-image" :src="single_media.url"/>
                                    </template>
                                    <template v-else>
                                        <video
                                         id="video-player"
                                         ...
                                         :data-cld-source="single_media.url"
                                        >
                                        </video>
                                    </template>
                                </div>
...
export default {
  ...
  mounted: function() {
    // Now we get all the related information for the particular product id

    axios
      .get(`http://localhost:3128/getProductInfo/${this.$route.params.id}`)
      .then(res => {
